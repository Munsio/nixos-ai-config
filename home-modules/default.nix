    ...
    let
      ...
      modulesList = lib.mapAttrsToList (name: _:
        let
          ...
          # For features, the option will be homeModules.name
          # For bundles and services, the option will be homeModules.category.name
          isFeature = category == "features";
          moduleName = if isFeature then name else "${category}.${name}";
        in utils.mkHomeModule {
          name = moduleName;
          description = "${name} ${category} home-manager module";
      ...
      type = lib.types.submodule {
        options = {
          ...
          bundles = lib.mkOption {
            type = lib.types.submodule { };
            default = { };
            description = "Set of home-manager bundle modules to enable";
          };
          ...
          services = lib.mkOption {
            type = lib.types.submodule { };
            default = { };
            description = "Set of home-manager service modules to enable";
          };
        };
      };
      default = { };
      description = "Set of home-manager modules to enable";
    };
  };
in modulesModule
